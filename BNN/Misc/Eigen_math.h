...
namespace BNN {
	...
	inline Tensor conv2df(const Tensor& a, const Tensor& b, shp2 st, shp2 pa, shp2 dil = 1) {
		idx d0 = b.dimension(0) / a.dimension(0);
		idx d1 = c_dim(a.dimension(1) * dil[0] - dil[0] + 1, b.dimension(1), st[0], pa[0]);
		idx d2 = c_dim(a.dimension(2) * dil[1] - dil[1] + 1, b.dimension(2), st[1], pa[1]);
		Tensor c(d0, d1, d2);
		conv2d(c, a, b, st, pa, dil);
		return c;
	}
