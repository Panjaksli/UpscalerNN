...
namespace BNN {
	...
	void conv2d_igrad(Tensor& inp, const Tensor& out, const Tensor& ker, shp2 st, shp2 pa, shp2 dil) {
		...
		dim1<2> kernel_dims{ ich, och * ker_w * ker_h};
		auto patch = out.extract_image_patches(ker_w, ker_h, 1, 1, dil[0], dil[1], st[0], st[1], pa[0], pa[0], pa[1], pa[1], 0)
			.reshape(dim1<2>{ker_w* ker_h* och, in_w* in_h});
	...
	void conv2d_wgrad(Tensor& ker, const Tensor& inp, const Tensor& out, shp2 st, shp2 pa, shp2 dil) {
		...
		idx out_w = out.dimension(1);
		idx out_h = out.dimension(2);
		ker += inp.extract_image_patches(ker_w, ker_h, st[0], st[1], dil[0], dil[1], 1, 1, pa[0], pa[0], pa[1], pa[1], 0).reshape(dim1<2>{ich * ker_w* ker_h, out_w * out_h })
			.contract(out.reshape(dim1<2>{och, out_w* out_h }), dim2<1>{shp2{ 1, 1 }}).shuffle(dim1<2>{1, 0})
			.reshape(ker.dimensions());
	}
	...
	//fixing a screwup where I was convolving incorrect filters in backprop, also reverse operation is included inhouse
	//stupid me
