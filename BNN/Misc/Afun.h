...
namespace BNN {
	...
	class Efun {
	public:
		enum Type { t_mse, t_mae, t_msae } type = t_mse;
		Efun() {}
		...
		struct msae {
			static float fx(float x, float y) { return 0.5f * (mse::fx(x, y) + mae::fx(x, y)); }
			static float dx(float x, float y) { return 0.5f * (mse::dx(x, y) + mae::dx(x, y)); }
			static constexpr Type type = t_msae;
		};
	};
	class Afun {
	public:
